"""Reusable v3 feature-building blocks for offline and live builders."""
...
def base_feature_columns_v3() -> tuple[str, ...]:
    return (
        "logret_1",
        "logret_3",
        "logret_12",
...
def one_m_feature_columns_v3() -> tuple[str, ...]:
    return (
        "one_m_count",
        "one_m_ret_mean",
        "one_m_ret_std",
...
def high_tf_feature_columns_v3(*, high_tfs: tuple[str, ...] = ("15m", "60m", "240m")) -> tuple[str, ...]:
    high: list[str] = []
    for tf in high_tfs:
...
def micro_feature_columns_v3() -> tuple[str, ...]:
...
def feature_columns_v3_contract(*, high_tfs: tuple[str, ...] = ("15m", "60m", "240m")) -> tuple[str, ...]:
    return tuple(
        _dedupe_preserve(
            list(base_feature_columns_v3())
            + list(one_m_feature_columns_v3())
            + list(high_tf_feature_columns_v3(high_tfs=high_tfs))
            + list(micro_feature_columns_v3())
        )
    )


def required_feature_columns_v3(*, high_tfs: tuple[str, ...] = ("15m", "60m", "240m")) -> tuple[str, ...]:
    micro = (
        "m_micro_available",
        "m_trade_events",
        "m_book_events",
...
def compute_base_features_v3(base_candles_frame: pl.DataFrame, *, tf: str, float_dtype: str) -> pl.DataFrame:
...
def cast_feature_output_v3(
    ...
    *,
    ...
    exprs: list[pl.Expr] = []
    contract_cols = set(feature_columns_v3_contract(high_tfs=high_tfs))
