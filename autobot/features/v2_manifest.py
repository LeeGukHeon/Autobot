"""Manifest helpers for features_v2."""
...
FEATURES_V2_MANIFEST_SCHEMA: dict[str, pl.DataType] = {
    ...
    "built_at": pl.Int64,
    "join_compared_rows": pl.Int64,
    "join_matched_rows": pl.Int64,
    "join_match_ratio": pl.Float64,
    "micro_available_ratio": pl.Float64,
    "trade_coverage_p50_ms": pl.Int64,
    "trade_coverage_p90_ms": pl.Int64,
    "book_coverage_p50_ms": pl.Int64,
    "book_coverage_p90_ms": pl.Int64,
    "tail_dropped_rows": pl.Int64,
    "prefilter_rows": pl.Int64,
    "postfilter_rows": pl.Int64,
}
...
def empty_manifest() -> pl.DataFrame:
    return pl.DataFrame([], schema=FEATURES_V2_MANIFEST_SCHEMA, orient="row")
        ...
        normalized_rows.append(
            {
                ...
                "join_compared_rows": _coerce_int(item.get("join_compared_rows")),
                "join_matched_rows": _coerce_int(item.get("join_matched_rows")),
                "join_match_ratio": _coerce_float(item.get("join_match_ratio")),
                "micro_available_ratio": _coerce_float(item.get("micro_available_ratio")),
                "trade_coverage_p50_ms": _coerce_int(item.get("trade_coverage_p50_ms")),
                "trade_coverage_p90_ms": _coerce_int(item.get("trade_coverage_p90_ms")),
                "book_coverage_p50_ms": _coerce_int(item.get("book_coverage_p50_ms")),
                "book_coverage_p90_ms": _coerce_int(item.get("book_coverage_p90_ms")),
                "tail_dropped_rows": _coerce_int(item.get("tail_dropped_rows")),
                "prefilter_rows": _coerce_int(item.get("prefilter_rows")),
                "postfilter_rows": _coerce_int(item.get("postfilter_rows")),
            }
        )
    return pl.DataFrame(normalized_rows, schema=FEATURES_V2_MANIFEST_SCHEMA, orient="row")
