"""Plan generator for Upbit 30-level orderbook collection."""
...
class Lob30PlanOptions:
    parquet_root: Path = Path("data/parquet")
    base_dataset: str = "lob30_v1"
    market_source_dataset: str | None = None
    output_path: Path = Path("data/collect/_meta/lob30_plan.json")
    quote: str = "KRW"
...
def generate_lob30_collection_plan(options: Lob30PlanOptions) -> dict[str, Any]:
    ...
    request_codes = [f"{market}.30" for market in selected_markets]
    plan = {
        "version": "lob30_plan_v1",
        ...
        "runtime_policy": {
            ...
            "dedupe_policy": "latest_by_market_ts_ms",
        },
        "constraints": {
            "endpoint": "wss://api.upbit.com/websocket/v1",
            "level_grouping_allowed": False,
            "expected_depth_levels": 30,
            "rate_limit_policy": {
        ...
        "summary": {
            ...
            "request_codes_count": len(request_codes),
            "requested_depth": 30,
        },
    }
