"""Manifest helpers for raw Upbit private websocket collection outputs."""
...
PRIVATE_WS_MANIFEST_SCHEMA: dict[str, pl.DataType] = {
    "run_id": pl.Utf8,
    "channel": pl.Utf8,
    "date": pl.Utf8,
    "hour": pl.Utf8,
...
def empty_private_ws_manifest() -> pl.DataFrame:
    return pl.DataFrame([], schema=PRIVATE_WS_MANIFEST_SCHEMA, orient="row")


def load_private_ws_manifest(path: Path) -> pl.DataFrame:
    if not path.exists():
        return empty_private_ws_manifest()
    return _align_columns(pl.read_parquet(path))


def save_private_ws_manifest(path: Path, frame: pl.DataFrame) -> None:
    aligned = _align_columns(frame)
    path.parent.mkdir(parents=True, exist_ok=True)
    aligned.write_parquet(path, compression="zstd")


def append_private_ws_manifest_rows(path: Path, rows: list[dict[str, Any]]) -> pl.DataFrame:
    if not rows:
        frame = load_private_ws_manifest(path)
        save_private_ws_manifest(path, frame)
        return frame

    incoming = normalize_private_ws_manifest_rows(rows)
    if path.exists():
        combined = pl.concat([load_private_ws_manifest(path), incoming], how="vertical")
    else:
        combined = incoming
    save_private_ws_manifest(path, combined)
    return combined


def normalize_private_ws_manifest_rows(rows: list[dict[str, Any]]) -> pl.DataFrame:
    ...
    return pl.DataFrame(normalized, schema=PRIVATE_WS_MANIFEST_SCHEMA, orient="row")
...
def _align_columns(frame: pl.DataFrame) -> pl.DataFrame:
    ...
    return normalize_private_ws_manifest_rows(rows)
