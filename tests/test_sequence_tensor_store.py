...
def test_build_sequence_tensor_store_skips_complete_past_date_via_date_completeness(tmp_path: Path, monkeypatch) -> None:
    parquet_root = tmp_path / "parquet"
    out_root = parquet_root / "sequence_v1"
    meta_root = out_root / "_meta"
    ...
    for anchor_ts_ms in (1577923200000, 1577923260000):
        ...
        [
            {
                ...
                "cache_validity_signature": "sig-ready-market",
            },
            {
                "market": "KRW-BTC",
                "date": "2020-01-02",
        ...
        json.dumps(
            {
                ...
                "dates": {
                    "2020-01-02": {
                        ...
                        "reused_anchor_count": 2,
                        "max_anchors_per_market": 2,
                        "complete": True,
                        "date_validity_signature": "sig-ready-date",
                    }
                },
            },
            ensure_ascii=False,
    ...
    def _boom(*args, **kwargs):
        raise AssertionError("_load_market_source_frames should not be called when date completeness is ready")

    monkeypatch.setattr(sequence_module, "_load_market_source_frames", _boom)
    monkeypatch.setattr(sequence_module, "_resolve_tensor_date_validity_signature", lambda **kwargs: "sig-ready-date")
    ...
    assert summary.reused_anchors == 2
    build_report = json.loads((meta_root / "build_report.json").read_text(encoding="utf-8"))
    assert build_report["date_reused_complete"] is True
