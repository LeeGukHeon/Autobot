...
def _seed_train_snapshot_close_contract(project_root: Path) -> None:
    ...
    _write_json(
        project_root / "data" / "collect" / "_meta" / "train_snapshot_close_latest.json",
        {
            ...
            "snapshot_root": str(project_root / "data" / "snapshots" / "data_platform" / "snapshot-cache-001"),
    ...
    driver_path.write_text(
        textwrap.dedent(
            ...
            if command_key == ("-m", "autobot.cli", "model", "inspect-runtime-dataset"):
                ...
                append_log({"command": "model inspect-runtime-dataset", "dataset_root": str(dataset_root)})
                print(json.dumps({
