...
- [x] 27. Implement `v5_fusion`
  ...
  Current implementation note:
  implemented in `autobot/models/train_v5_fusion.py` and wired through `autobot/models/__init__.py` plus `autobot.cli model train --trainer v5_fusion`. The current fusion trainer now consumes aligned panel/sequence/LOB/**tradability** expert prediction tables, including matching runtime-export tables for the same runtime window, and fits the stage-1 fusion meta-model on the blueprint-aligned output contract `final_rank_score`, `final_expected_return`, `final_expected_es`, `final_tradability`, `final_uncertainty`, and `final_alpha_lcb`. The implementation now supports `linear`, `monotone_gbdt`, and `regime_moe` stackers, persists `fusion_model_contract.json`, `predictor_contract.json`, `entry_boundary_contract.json`, `fusion_runtime_input_contract.json`, `domain_weighting_report.json`, and `ood_generalization_report.json`, and records the selected `sequence_variant_name`, `lob_variant_name`, `fusion_variant_name`, `tradability_source_run_id`, and domain-weighting provenance in the runtime recommendation / reporting surface rather than leaving those inputs implicit. Governed v5 acceptance now also treats `v5_tradability` as a required dependency trainer, requires `fusion_tradability_input` plus `fusion_tradability_runtime_input`, and validates the chosen sequence / LOB / fusion variant reports as part of the operational contract rather than only training the old panel/sequence/LOB three-expert path. Local validation covered `tests/test_train_v5_tradability.py`, `tests/test_v5_variant_selection.py`, `tests/test_train_v5_fusion.py`, `tests/test_candidate_acceptance_v5_dependency_inputs.py`, `tests/test_candidate_acceptance_dependency_reuse.py`, `tests/test_candidate_acceptance_certification_lane.py`, `tests/test_runtime_recommendations.py`, `tests/test_candidate_canary_report.py`, `tests/test_dashboard_server.py`, `tests/test_data_platform_snapshot.py`, and the adjacent trainer / CLI regression suites; direct OCI validation in the current follow-up session also re-passed the same server-side target suites after pulling the newer v5 tradability / snapshot / script-contract work. Remaining limitation retained explicitly: the current `ood_generalization_report` is still an informative weighting/provenance artifact around inverse-frequency domain weighting, not true FOIL/IRM-style invariant learning, so the stronger blueprint `domain shift aware / invariant` end-state still remains future methodology work rather than something silently claimed as already implemented.
