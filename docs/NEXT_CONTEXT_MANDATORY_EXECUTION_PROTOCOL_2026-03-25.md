...
- [x] 02. Make `features_v4` validation artifact mandatory in the operational flow
  ...
  Current implementation note:
  candidate acceptance now enforces both `features validate` and a sampled `live_feature_parity_report` in the operational flow, so the gate fails not only on missing/invalid `validate_report` but also when the live feature provider cannot reconstruct sampled `features_v4` rows without missing-column hard gates or row-level parity mismatches. The main operational wrappers still route through `candidate_acceptance.ps1`, server-side stale partitions causing the prior `ctrend_v1_rsi_14` schema failure were archived, and the acceptance flow now records a dedicated `features_live_parity` step alongside `features_validate` to cover the blueprint-required `missing feature parity pass` condition rather than only `fail_files/schema_ok/leakage_smoke`. Later parity-hardening work also aligned the certification path to the actual offline contract by preferring canonical multi-tf candles over naive `1m` rollups, using the full manifest universe plus micro-required context for cross-sectional features, widening history lookback for rolling `order_flow`/`trend_volume` features, and loading the full offline micro snapshot field set rather than a shallow subset.
...
- [x] 27. Implement `v5_fusion`
  ...
  Current implementation note:
  implemented in `autobot/models/train_v5_fusion.py` and wired through `autobot/models/__init__.py` plus `autobot.cli model train --trainer v5_fusion`. The new fusion trainer consumes aligned panel/sequence/LOB expert prediction tables, collects them into a shared OOF-style fusion input matrix keyed by `(market, ts_ms, split)`, then fits the stage-1 fusion meta-model using the blueprint-aligned output contract `final_rank_score`, `final_expected_return`, `final_expected_es`, `final_tradability`, `final_uncertainty`, and `final_alpha_lcb`. The current implementation supports the practical first-stage fusion choice `linear` as well as `monotone_gbdt`, writes `fusion_model_contract.json`, `predictor_contract.json`, `entry_boundary_contract.json`, a runtime-loadable fusion feature dataset, and the required runtime/governance artifacts so the family can participate in the same promotion/adoption lifecycle instead of remaining an offline-only stub. The live/paper runtime now also has a `LIVE_V5` provider path that reconstructs the required panel/sequence/LOB expert inputs online, so `train_v5_fusion` no longer depends on a panel-only primary-slot contract. Local validation covered `tests/test_train_v5_fusion.py`, `tests/test_train_v5_lob.py`, `tests/test_train_v5_sequence.py`, `tests/test_cli_alpha_shortcuts.py`, `tests/test_sequence_tensor_store.py`, and `tests/test_predictor_contract.py`; direct OCI validation had previously confirmed the server-side suite passes on the server and an additional synthetic CLI smoke run under `train_v5_fusion_smoke` produced a real reflected run directory with `fusion_model_contract.json`, `predictor_contract.json`, `stacker_family=linear`, the expected final output fields, and the explicit expert inputs `panel`, `sequence`, and `lob`. Further hardening on 2026-04-01 aligned the nightly `train_snapshot_close -> candidate_acceptance -> v5_fusion` contract by making train-close publish explicit `train_window / certification_window / coverage_window`, forcing `features_v4` refresh under the full coverage window, making acceptance consume frozen close artifacts for the v5 path when the close report exposes them, resolving and persisting a machine-readable `common_runtime_universe.json`, enforcing same snapshot/window/universe before fusion training, and extending expert export plus fusion runtime contracts with coverage timestamps and common-universe metadata. Direct OCI validation on 2026-04-01 pulled commit `cbddcdaa8a42d4b3399c3d876477bd6a27b41466` and passed the server-side suite `tests/test_train_v5_fusion.py tests/test_candidate_acceptance_v5_dependency_inputs.py tests/test_candidate_acceptance_dependency_reuse.py tests/test_candidate_acceptance_runtime_coverage.py tests/test_candidate_acceptance_certification_lane.py tests/test_cli_export_expert_table.py tests/test_t23_2_server_script_contracts.py -q` under `.venv/bin/python`. Remaining limitation retained explicitly: the current fusion path still uses the currently available tradability-like signals from the expert prediction tables, primarily the panel/LOB/sequence exported tradability proxies, because a separately materialized tradability expert family is not yet implemented in the current checklist slice. That is an explicit remaining architecture gap rather than a hidden scope reduction.

- [x] 28. Add risk-calibrated or conformal entry boundary
  Required references:
  [TRAINING_MODEL_STRENGTHENING_BLUEPRINT_2026-03-25.md](/d:/MyApps/Autobot/docs/TRAINING_MODEL_STRENGTHENING_BLUEPRINT_2026-03-25.md)
  [RISK_AND_LIVE_CONTROL_STRENGTHENING_BLUEPRINT_2026-03-25.md](/d:/MyApps/Autobot/docs/RISK_AND_LIVE_CONTROL_STRENGTHENING_BLUEPRINT_2026-03-25.md)

  Current implementation note:
  implemented by adding `autobot/models/entry_boundary.py`, extending `autobot/models/train_v5_fusion.py`, `autobot/models/predictor.py`, and `autobot/strategy/model_alpha_v1.py`, and validating the new contract through both trainer-side and runtime-side tests. The current fusion trainer now writes `entry_boundary_contract.json` as a machine-readable post-hoc risk-calibrated entry boundary artifact on top of the fused outputs, calibrating a severe-loss-risk threshold from validation-split data while preserving the white-box conjunctive contract shape `alpha_lcb > floor`, `tradability >= threshold`, and `estimated_severe_loss_risk <= threshold`. Runtime predictor loading now reads that boundary artifact, and `model_alpha_v1` now appends the missing `final_expected_return`, `final_expected_es`, `final_tradability`, and `final_alpha_lcb` columns to scored rows before evaluating the boundary, so blocked entries become explicit strategy-level skip reasons such as `ENTRY_BOUNDARY_ALPHA_LCB_NOT_POSITIVE`, `ENTRY_BOUNDARY_TRADABILITY_BELOW_THRESHOLD`, and `ENTRY_BOUNDARY_SEVERE_LOSS_RISK_HIGH` instead of remaining hidden in later downstream gates. Local validation covered `tests/test_entry_boundary.py`, `tests/test_train_v5_fusion.py`, `tests/test_train_v5_lob.py`, `tests/test_train_v5_sequence.py`, `tests/test_cli_alpha_shortcuts.py`, `tests/test_sequence_tensor_store.py`, and `tests/test_predictor_contract.py`; direct OCI validation confirmed the same suite passes on the server and a synthetic CLI smoke run under `train_v5_fusion_smoke` produced a reflected `entry_boundary_contract.json` with `policy=risk_calibrated_entry_boundary_v1` alongside the fusion model and predictor contracts. Open note retained explicitly: the current boundary calibration still uses the presently available fused expert outputs and realized-return severe-loss proxy rather than a richer future dedicated tradability expert or full conformal risk-training stack, so this is the intended blueprint `post-hoc risk-calibrated threshold` starting point rather than the eventual strongest end-state.
...
- [x] 30. Integrate promote/abort/continue state machine into automation
  Required references:
  [SERVER_OPERATIONS_AND_DEPLOYMENT_AUTOMATION_BLUEPRINT_2026-03-25.md](/d:/MyApps/Autobot/docs/SERVER_OPERATIONS_AND_DEPLOYMENT_AUTOMATION_BLUEPRINT_2026-03-25.md)
  [FULLY_AUTOMATED_CHAMPION_CANDIDATE_OPERATING_CONTRACT_2026-03-24.md](/d:/MyApps/Autobot/docs/FULLY_AUTOMATED_CHAMPION_CANDIDATE_OPERATING_CONTRACT_2026-03-24.md)

  Current implementation note:
  implemented by extending `scripts/daily_champion_challenger_v4_for_server.ps1` with an explicit `promote | abort | continue` resolver, canary-artifact lookup, and a persisted promotion state-machine artifact at `logs/model_v4_challenger/step_06_promote.json`. `promote_only` no longer treats the previous candidate as a binary pass/fail only; it first consumes paired-paper evidence, then reads `logs/canary_confidence_sequence/<candidate-unit>/latest.json`, and resolves a machine-readable action. `promote` preserves the existing cutover semantics; `abort` now explicitly rejects the candidate by stopping candidate evidence targets, clearing `latest_candidate`, and removing `logs/model_v4_challenger/current_state.json`; `continue` keeps the current champion unchanged, preserves both `latest_candidate` and `current_state.json`, and resumes challenger paper plus candidate canary evidence collection when the previous candidate needs more evidence rather than rejection. Targeted automation validation covered `tests/test_daily_champion_challenger_spawn_handling.py` scenarios for promote success, paired-paper continue, explicit abort, and spawn-then-promote end-to-end state handoff, plus the relevant canary/dashboard/fusion guard tests that share the same promotion evidence surface. Direct OCI validation additionally confirmed the targeted state-machine suite passes server-side after deployment, so the promote timer now follows the same explicit state contract that the new canary artifact exposes instead of silently collapsing `continue` into `fail`.
