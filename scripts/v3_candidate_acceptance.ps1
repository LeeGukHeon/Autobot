param(
    ...
    [string]$OutDir = "logs/model_v3_acceptance",
    ...
    [int]$Seed = 42,
    [int]$NThread = 1,
    [double]$BacktestTopPct = 0.10,
    [double]$BacktestMinProb = 0.52,
    [int]$BacktestMinCandidatesPerTs = 3,
    [int]$HoldBars = 6,
    ...
    [double]$PaperMaxFallbackRatio = 0.10,
...
try {
    if (-not $SkipDailyPipeline) {
        ...
        if ($dailyExec.ExitCode -ne 0) {
            ...
            $paths = Save-Report
            Write-Host ("[v3-accept] overall_pass=False report={0}" -f $paths.LatestReportPath)
            exit 2
        }
    } else {
    ...
    $candidateBacktest = Invoke-BacktestAndLoadSummary -PythonPath $resolvedPythonExe -Root $resolvedProjectRoot -ModelRef "latest_candidate_v3" -StartDate $backtestStartDate -EndDate $effectiveBatchDate
    ...
    if (-not [string]::IsNullOrWhiteSpace($championRunId)) {
        $championBacktest = Invoke-BacktestAndLoadSummary -PythonPath $resolvedPythonExe -Root $resolvedProjectRoot -ModelRef "champion_v3" -StartDate $backtestStartDate -EndDate $effectiveBatchDate
    ...
    if ($championCompareEvaluated) {
        $championDeltaPass = ($candidateRealizedPnl - $championRealizedPnl) -ge $BacktestMinPnlDeltaVsChampion
    }
    ...
    $overallPass = $backtestPass -and $paperPass
    $reasons = @()
    if (-not $backtestPass) {
    ...
    if ($overallPass -and (-not $SkipPromote)) {
        $promoteArgs = @(
            "-m", "autobot.cli",
            "model", "promote",
    ...
    $paths = Save-Report
    Write-Host ("[v3-accept] candidate_run_id={0}" -f $candidateRunId)
    Write-Host ("[v3-accept] backtest_pass={0}" -f $backtestPass)
    Write-Host ("[v3-accept] paper_pass={0}" -f $paperPass)
    Write-Host ("[v3-accept] overall_pass={0}" -f $overallPass)
    Write-Host ("[v3-accept] report={0}" -f $paths.RunReportPath)
    Write-Host ("[v3-accept] latest={0}" -f $paths.LatestReportPath)
    if ($DryRun) {
        exit 0
    }
    if ($overallPass) {
    ...
    $paths = Save-Report
    Write-Host ("[v3-accept][error] {0}" -f $_.Exception.Message)
    Write-Host ("[v3-accept] latest={0}" -f $paths.LatestReportPath)
    exit 2
}
